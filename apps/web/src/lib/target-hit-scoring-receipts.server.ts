import { CALL_ID, RECEIPT_ID, receipt, receiptPage, type ReceiptState } from "./target-hit-scoring-receipts";

type Options = { mode?: string; baseUrl?: string; fetcher?: typeof fetch };
const MAX_BYTES = 1048576;
export async function loadTargetHitScoringReceipts(callId: string, selectedId: string | undefined, options: Options = {}): Promise<ReceiptState> {
  ...
  const mode = options.mode ?? process.env.TARGET_HIT_SCORING_RECEIPTS_PROVIDER ?? "disabled";
  ...
  try {
    ...
    const route = `v1/calls/${encodeURIComponent(callId)}/target-hit-scoring-receipts${selectedId ? `/${selectedId}` : ""}`;
