...
import { sampleReceipt } from "@/lib/comparative-scoring-receipts.fixture";
import type { ReceiptState } from "@/lib/comparative-scoring-receipts";
...
describe("receipt audit presentation", () => {
  ...
  it("empty explicitly says this page does not create records", () => {
    ...
    expect(screen.getByText("이 DEMO 발언에 저장된 비교 평가 기록이 없습니다.")).toBeVisible();
  ...
  it("renders exact values and evidence without implying complete or signed data", () => {
    ...
    expect(screen.getByRole("region",{name:"비교 평가 기록 표 (가로 스크롤)"})).toHaveAttribute("tabindex","0");
