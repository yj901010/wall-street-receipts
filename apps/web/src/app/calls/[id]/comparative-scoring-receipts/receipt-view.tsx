...
import type { Metric, ReceiptState, ComparativeScoringReceipt, ReferenceEvidence } from "@/lib/comparative-scoring-receipts";
...
function Evidence({ row, messages: m }: { row: ComparativeScoringReceipt; messages: Messages }) {
...
export function ReceiptView({ callId, selectedId, state, locale }: { callId: string; selectedId?: string; state: ReceiptState; locale: "ko" | "en" }) {
  const m = scoringMessages(locale); const base = `/calls/${encodeURIComponent(callId)}/comparative-scoring-receipts`;
  ...
  return <main><SiteHeader current="calls" dataMode="DEMO" /><div className={`page-shell ${styles.shell}`}>
    ...
    <header className={styles.heading}><p className="eyebrow">DEMO / PARTIAL_COMPARATIVE</p><h1>{m.title}</h1><p className={styles.identifier}>{callId}</p></header>
    ...
    {!ready ? <section role={state.kind === "invalid" || state.kind === "unavailable" ? "alert" : "status"} className={styles.feedback}>
      ...
      <section aria-labelledby="receipt-list"><h2 id="receipt-list">{state.selected ? m.selected : m.list}</h2>
        ...
        <div role="region" aria-label={m.scroll} tabIndex={0} className={styles.scroll}>
          <table className={styles.table}><thead><tr>{[m.id, m.basis, m.asof, m.recorded, m.asset, m.direction, m.error, m.benchmark, m.sector].map(label => <th scope="col" key={label}>{label}</th>)}</tr></thead>
            <tbody>{state.items.map(row => <tr key={row.receiptId}>
              ...
              <td><MetricValue metric={row.assetReturn} messages={m} /></td><td><MetricValue metric={row.directionalWin} messages={m} /></td><td><MetricValue metric={row.targetError} messages={m} /></td><td><MetricValue metric={row.benchmarkReturn} messages={m} /></td><td><MetricValue metric={row.sectorReturn} messages={m} /></td>
            </tr>)}</tbody></table>
        </div>{state.hasMore && <p className={styles.note}>{m.more}</p>}
      </section>{state.selected && <><Evidence row={state.items[0]} messages={m} /><p className={styles.note}>{m.referenceNotice}</p><Reference value={state.items[0].benchmarkEvidence} title={m.benchmarkEvidence} id="benchmark-evidence" messages={m} /><Reference value={state.items[0].sectorEvidence} title={m.sectorEvidence} id="sector-evidence" messages={m} /></>}
    </>}
