...
public record OperatorApiProperties(
        boolean enabled,
        String tokenSha256,
        @DefaultValue("FULL") String access) {

    private static final Pattern SHA_256 = Pattern.compile("^[0-9a-f]{64}$");
    private static final String ALL_ZERO_SHA_256 = "0".repeat(64);

    public OperatorApiProperties {
        if (!"FULL".equals(access) && !"CPI_READ_ONLY".equals(access)) {
            throw new IllegalArgumentException("OPERATOR_API_ACCESS must be FULL or CPI_READ_ONLY");
        }
    ...
    public boolean cpiReadOnly() {
        return "CPI_READ_ONLY".equals(access);
    }

    @Override
    public String toString() {
        return "OperatorApiProperties[enabled=" + enabled + ", tokenSha256=<redacted>, access=" + access + "]";
    }
}
