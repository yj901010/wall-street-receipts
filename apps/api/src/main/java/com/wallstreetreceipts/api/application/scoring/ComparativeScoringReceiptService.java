...
import com.wallstreetreceipts.api.application.port.out.ComparativeScoringReceiptRepository;
...
@Service
public class ComparativeScoringReceiptService {
    private final ComparativeScoringReceiptRepository repository;
    ...
    public ComparativeScoringReceiptService(ComparativeScoringReceiptRepository repository, ScoringLedgerVerifier ledger, AnalystCallRepository calls, Clock clock) {
        this.repository = repository; this.ledger = ledger; this.calls = calls; this.clock = clock;
    }
    public record Verified(Entry stored, ComparativeScoringEvaluator.Receipt evaluation, String snapshotProvenanceId) {}
    ...
    public Verified append(ComparativeScoringInput input, String snapshotId) {
        String callId = input.endpoint().horizon().basis().callId();
        ...
        var binding = ledger.verify(input.endpoint(), snapshotId);
        ...
        if (recordedAt.isBefore(input.endpoint().evaluationAsOf())) throw new IllegalArgumentException("Evaluation cannot be in the future");
        var entry = new Entry(UUID.randomUUID(), callId, snapshotId, input.endpoint().horizon().basis().basisRevisionId(), binding.revisionSequence(),
                evaluated.methodologyId(), evaluated.methodologyVersion(), evaluated.methodologyDefinitionHash(),
                ComparativeScoringMethodology.canonicalDefinition(), evaluated.inputFingerprint(), binding.fingerprint(),
                input.endpoint().evaluationAsOf(), recordedAt, ComparativeScoringInputCodec.encode(input));
    ...
    private Verified replay(Entry row) {
        ...
        var input = ComparativeScoringInputCodec.decode(bytes);
        ...
        if (!row.callId().equals(basis.callId()) || !Objects.equals(row.basisRevisionId(), basis.basisRevisionId())
                || !row.evaluationAsOf().equals(input.endpoint().evaluationAsOf()) || row.recordedAt().isBefore(row.evaluationAsOf())
                || !row.inputFingerprint().equals(ComparativeScoringInputCodec.hash(bytes))
                || !row.methodologyDefinition().equals(ComparativeScoringMethodology.canonicalDefinition())) throw new Unavailable();
        var binding = ledger.verify(input.endpoint(), row.snapshotId());
