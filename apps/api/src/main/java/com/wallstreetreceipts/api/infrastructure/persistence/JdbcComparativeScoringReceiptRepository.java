...
@Repository
public class JdbcComparativeScoringReceiptRepository implements ComparativeScoringReceiptRepository {
    private final JdbcTemplate jdbc;
    public JdbcComparativeScoringReceiptRepository(javax.sql.DataSource source) {
    ...
    @Override public void insert(Entry e) {
        jdbc.update("""
                INSERT INTO demo_comparative_scoring_receipts(receipt_id,call_id,snapshot_id,basis_revision_id,basis_revision_sequence,basis_revision_type,
    ...
    @Override public Optional<Entry> findByIdentity(String input, String ledger) {
        return jdbc.query("SELECT * FROM demo_comparative_scoring_receipts WHERE input_fingerprint=? AND ledger_fingerprint=?", this::row, input, ledger).stream().findFirst();
    }
    @Override public Optional<Entry> find(String call, UUID id) {
        return jdbc.query("SELECT * FROM demo_comparative_scoring_receipts WHERE call_id=? AND receipt_id=?", this::row, call, id.toString()).stream().findFirst();
    }
    @Override public List<Entry> recent(String call) {
        return jdbc.query("SELECT * FROM demo_comparative_scoring_receipts WHERE call_id=? ORDER BY recorded_at DESC, receipt_id DESC LIMIT 21", this::row, call);
    }
    private Entry row(ResultSet r, int n) throws SQLException {
        if (!"DEMO".equals(r.getString("data_mode")) || !"PARTIAL_COMPARATIVE".equals(r.getString("receipt_scope")) || r.getBoolean("data_complete")) {
