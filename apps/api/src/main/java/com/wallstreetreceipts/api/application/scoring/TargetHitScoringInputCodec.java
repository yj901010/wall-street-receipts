...
/** Closed canonical format. Never loads a class named by input, never changes ADR-080 or ADR-083 bytes. */
public final class TargetHitScoringInputCodec {
    private TargetHitScoringInputCodec() {}
    private static final int MAX_BYTES = 1048576;
    private static final String FORMAT = "wsr-target-hit-input-v1";
    private static final Map<String, Class<?>> TYPES;
    static {
        var types = new HashMap<String, Class<?>>();
        for (var type : List.of(TargetHitScoringInput.class, WindowPriceBinding.class, FullWindowHighLowObservation.class,
                OutcomeBasis.Original.class, OutcomeBasis.Correction.class, OutcomeHorizon.class,
                FullWindowHighLowObservation.BoundaryType.class, FullWindowHighLowObservation.WindowPriceField.class,
                FullWindowHighLowObservation.WindowCoverageCompleteness.class,
                CorporateActionContinuity.class, EndpointPriceAdjustmentBasis.class)) {
    ...
    public static byte[] encode(TargetHitScoringInput input) {
        Objects.requireNonNull(input, "input");
        var bytes = new ByteArrayOutputStream();
        try (var out = new DataOutputStream(bytes)) {
            var encoder = new Encoder(bytes, out);
            for (String header : List.of(FORMAT, TargetHitScoringMethodology.ID, TargetHitScoringMethodology.VERSION,
                    TargetHitScoringMethodology.definitionHash())) encoder.value(header);
            encoder.value(input);
            return bytes.toByteArray();
        } catch (IOException | ReflectiveOperationException failure) { throw invalid(); }
    }

    public static TargetHitScoringInput decode(byte[] bytes) {
        Objects.requireNonNull(bytes, "bytes");
        if (bytes.length == 0 || bytes.length > MAX_BYTES) throw invalid();
        try (var in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            for (String header : List.of(FORMAT, TargetHitScoringMethodology.ID, TargetHitScoringMethodology.VERSION,
                    TargetHitScoringMethodology.definitionHash())) if (!header.equals(read(in, 0))) throw invalid();
            Object value = read(in, 0);
            if (!(value instanceof TargetHitScoringInput input) || in.available() != 0
    ...
    private record Encoder(ByteArrayOutputStream bytes, DataOutputStream out) {
        void value(Object value) throws IOException, ReflectiveOperationException {
            ...
            } else if (value instanceof ComparativeScoringInput comparative) {
                out.writeByte(8);
                byte[] encoded = ComparativeScoringInputCodec.encode(comparative);
        ...
        return switch (in.readUnsignedByte()) {
            ...
            case 2 -> {
                ...
                yield Arrays.stream(type.getEnumConstants()).filter(e -> ((Enum<?>) e).name().equals(name))
                        .findFirst().orElseThrow(TargetHitScoringInputCodec::invalid);
            }
            ...
            case 8 -> {
                ...
                yield ComparativeScoringInputCodec.decode(in.readNBytes(length));
            }
    ...
    private static IllegalArgumentException invalid() { return new IllegalArgumentException("Invalid target-hit scoring input encoding"); }
}
