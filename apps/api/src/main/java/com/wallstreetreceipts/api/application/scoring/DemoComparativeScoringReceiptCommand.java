...
public final class DemoComparativeScoringReceiptCommand {
    private DemoComparativeScoringReceiptCommand() {}
    ...
    public static int run(String[] args, Map<String, String> environment, PrintStream output) {
        ComparativeScoringInput input;
        String snapshot;
        PGSimpleDataSource source;
        try {
            if (args.length != 3 || !args[0].equals("--confirm=APPEND_DEMO_COMPARATIVE_SCORING_RECEIPT")
            ...
            String url = environment.getOrDefault("WSR_DEMO_COMPARATIVE_SCORING_JDBC_URL", "");
            var match = java.util.regex.Pattern.compile("jdbc:postgresql://127\\.0\\.0\\.1:([1-9][0-9]{3,4})/wsr_comparative_scoring_demo").matcher(url);
            ...
            String user = environment.getOrDefault("WSR_DEMO_COMPARATIVE_SCORING_USER", "");
            String password = environment.getOrDefault("WSR_DEMO_COMPARATIVE_SCORING_PASSWORD", "");
            ...
            try (var stream = Files.newInputStream(path, LinkOption.NOFOLLOW_LINKS)) {
                input = ComparativeScoringInputCodec.decode(stream.readNBytes(1048577));
            }
            source = new PGSimpleDataSource(); source.setURL(url); source.setUser(user); source.setPassword(password);
            source.setConnectTimeout(3); source.setSocketTimeout(5); source.setApplicationName("wsr-demo-comparative-scoring-append");
        } catch (Exception invalid) {
            output.println("DEMO_COMPARATIVE_SCORING_REJECTED: explicit confirmation, bounded canonical input and local DEMO configuration required");
            return 64;
        }
        try {
            ...
            var service = new ComparativeScoringReceiptService(new JdbcComparativeScoringReceiptRepository(source), new ScoringLedgerVerifier(calls, revisions), calls, Clock.systemUTC());
            var transaction = new TransactionTemplate(new DataSourceTransactionManager(source));
            transaction.setTimeout(10);
            var saved = Objects.requireNonNull(transaction.execute(status -> service.append(input, snapshot)));
            output.println("DEMO_COMPARATIVE_SCORING_RECEIPT: " + saved.stored().receiptId() + " " + saved.stored().inputFingerprint());
            return 0;
        } catch (RuntimeException failure) {
            // A connection failure at commit is not proof of rollback. Retry the same input, never invent a new as-of.
            output.println("DEMO_COMPARATIVE_SCORING_UNCONFIRMED: verify or retry the identical input; no successful append is claimed");
            return 69;
        }
    }
}
