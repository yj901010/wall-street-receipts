...
CREATE TABLE demo_target_hit_scoring_receipts (
    ...
    receipt_scope VARCHAR(32) NOT NULL DEFAULT 'PARTIAL_TARGET_HIT' CHECK (receipt_scope = 'PARTIAL_TARGET_HIT'),
    data_complete BOOLEAN NOT NULL DEFAULT FALSE CHECK (data_complete = FALSE),
    CONSTRAINT fk_target_hit_snapshot FOREIGN KEY (call_id, snapshot_id)
        REFERENCES market_snapshots(call_id, snapshot_id) ON DELETE RESTRICT,
    CONSTRAINT fk_target_hit_correction FOREIGN KEY (call_id, basis_revision_id, basis_revision_sequence, basis_revision_type)
        REFERENCES analyst_call_revisions(call_id, revision_id, sequence_number, revision_type) ON DELETE RESTRICT,
    CONSTRAINT ck_target_hit_basis CHECK (
    ...
    CONSTRAINT ck_target_hit_time CHECK (recorded_at >= evaluation_as_of),
    CONSTRAINT ck_target_hit_bytes CHECK (OCTET_LENGTH(input_bytes) BETWEEN 1 AND 1048576),
    CONSTRAINT uq_target_hit_input_ledger UNIQUE (input_fingerprint, ledger_fingerprint)
);
CREATE INDEX idx_target_hit_call_recorded ON demo_target_hit_scoring_receipts(call_id, recorded_at DESC, receipt_id DESC);
