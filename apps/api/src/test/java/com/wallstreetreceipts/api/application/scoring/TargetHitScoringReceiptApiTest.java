...
@SpringBootTest(properties = "spring.datasource.url=jdbc:h2:mem:target-hit-scoring-receipts;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1")
@ActiveProfiles("test") @AutoConfigureMockMvc @Transactional
@Import(TargetHitScoringReceiptApiTest.FixedClock.class)
class TargetHitScoringReceiptApiTest {
    static final String PATH = "/v1/calls/demo-call/target-hit-scoring-receipts";
    @Autowired TargetHitScoringReceiptService service;
    @Autowired TargetHitScoringReceiptRepository receipts;
    ...
    @Test void appendReplaysAndRetainsOriginalBytesTimeAndIdentityWithoutTreatingSnapshotAsPrice() throws Exception {
        ...
        assertThat(replay.stored().inputBytes()).isEqualTo(TargetHitScoringInputCodec.encode(TargetHitScoringFixture.input(input, "160", "80")));
        var bytes = replay.stored().inputBytes(); bytes[0] = 0;
        assertThat(replay.stored().inputBytes()[0]).isEqualTo((byte) 1);
        var response = TargetHitScoringReceiptResponse.from(replay);
        ...
        assertThat(response.targetHit().booleanValue()).isTrue();
        assertThat(response.windowEvidence().selectedField()).isEqualTo("HIGH");
        assertThat(response.windowEvidence().selectedValue()).isEqualTo("160");
        assertThat(response.windowEvidence().observation().windowLow()).isEqualTo("80");
        ...
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM demo_comparative_scoring_receipts", Integer.class)).isZero();
    ...
    @Test void changedInputAppendsNewReceiptAndRecentIsBoundedWithDeterministicTies() {
        ...
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM demo_target_hit_scoring_receipts", Integer.class)).isEqualTo(23);
    }
    @Test void correctionRequiresActualSameCallRevisionAndRoundTripsItsSequence() {
        ...
        assertThat(service.find(call.id(), stored.receiptId()).evaluation().input().comparative().endpoint().horizon().basis().basisRevisionId()).isEqualTo(r.id());
        assertThat(TargetHitScoringInputCodec.encode(TargetHitScoringInputCodec.decode(stored.inputBytes()))).isEqualTo(stored.inputBytes());
    }
    ...
    @Test void missingTargetStaysMissingAndCorrectionCaptureAfterAsOfIsRejected() {
        ...
        var response = TargetHitScoringReceiptResponse.from(append(absent, "receipt-snapshot"));
        assertThat(response.targetError().decimalValue()).isNull();
        assertThat(response.targetError().reasons()).contains("TARGET_MISSING_AS_OF");
        assertThat(response.targetHit().state()).isEqualTo("NOT_APPLICABLE");
        assertThat(response.targetHit().reasons()).containsExactly("TARGET_ABSENT");
        assertThat(response.targetHit().booleanValue()).isNull();
    ...
    void metadataDriftFailsClosedWithoutEchoOrListFallback(String column) throws Exception {
        append(input, "receipt-snapshot");
        jdbc.update("UPDATE demo_target_hit_scoring_receipts SET " + column + "=?", "x");
        unavailable();
    }
    @ParameterizedTest @ValueSource(strings = {"1-1-1-1-1", "ABCDEFAB-CDEF-ABCD-EFAB-CDEFABCDEFAB"})
    void corruptStoredUuidIsRejectedInsteadOfPublishingANormalizedIdentity(String id) throws Exception {
        append(input, "receipt-snapshot");
        jdbc.update("UPDATE demo_target_hit_scoring_receipts SET receipt_id=?", id);
        unavailable();
    }
    ...
    void payloadAndLedgerDriftAreNeverPublished(String field) throws Exception {
        append(input, "receipt-snapshot");
        switch (field) {
            case "bytes" -> jdbc.update("UPDATE demo_target_hit_scoring_receipts SET input_bytes=?", new byte[]{1,2,3});
            ...
            case "asof" -> jdbc.update("UPDATE demo_target_hit_scoring_receipts SET evaluation_as_of=?", java.sql.Timestamp.from(AS_OF.minusSeconds(1)));
    ...
    private TargetHitScoringReceiptService.Verified append(EndpointScoringInput endpoint, String snapshot) {
        return service.append(TargetHitScoringFixture.input(endpoint, "160", "80"), snapshot);
    }
    ...
    @Test void emptyMissingCrossCallInvalidQueriesAndNoHttpWriter() throws Exception {
        ...
        mvc.perform(get("/v1/calls/demo-call-002/target-hit-scoring-receipts/" + id)).andExpect(status().isNotFound());
    ...
    @Test void nullEvidenceIsNotZeroAndNeutralRemainsNotApplicableWhileReturnIsPending() {
        ...
        var response = TargetHitScoringReceiptResponse.from(append(pending, "receipt-snapshot"));
        assertThat(response.assetReturn().state()).isEqualTo("PENDING");
        assertThat(response.assetReturn().decimalValue()).isNull();
        assertThat(response.targetHit().state()).isEqualTo("NOT_APPLICABLE");
        assertThat(response.targetHit().booleanValue()).isNull();
        assertThat(response.windowEvidence()).isNull();
        ...
        assertThat(TargetHitScoringReceiptResponse.from(append(incomplete, "receipt-snapshot")).assetReturn().state()).isEqualTo("UNAVAILABLE");
    }

    @Test void changedWindowEvidenceAppendsWithoutRewritingOlderProfilesAndTamperingFails() throws Exception {
        var original = TargetHitScoringFixture.input(input, "160", "80");
        var first = service.append(original, "receipt-snapshot");
        var changed = edit(original, "windowCandidates", List.of());
        ...
        var replay = TargetHitScoringReceiptResponse.from(service.find(call.id(), second.stored().receiptId()));
        assertThat(replay.targetHit().state()).isEqualTo("UNAVAILABLE");
        assertThat(replay.targetHit().booleanValue()).isNull();
        assertThat(replay.windowEvidence()).isNull();
        assertThat(replay.benchmarkReturn().decimalValue()).isEqualTo("0.100000000000");
        ...
        jdbc.update("UPDATE demo_target_hit_scoring_receipts SET input_bytes=? WHERE receipt_id=?",
