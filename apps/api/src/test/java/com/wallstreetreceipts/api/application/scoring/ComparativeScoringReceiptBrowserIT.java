...
class ComparativeScoringReceiptBrowserIT {
    @Test void ownedDemoCommandAndReadOnlyAuditWorkflow() throws Exception {
        assertThat(System.getProperty("wsr.comparative.scoring.browser.confirm")).isEqualTo("DISPOSABLE_DEMO_ONLY");
        Path root=Path.of("").toAbsolutePath().getParent().getParent();
        Path web=Path.of(System.getProperty("wsr.comparative.scoring.browser.web")).toRealPath();
        assertThat(web.startsWith(root.resolve(".cache").toRealPath())).isTrue();
        for(var tree:List.of("src","public","operator","scoring","comparative-scoring","e2e")) sameTree(root.resolve("apps/web/"+tree),web.resolve(tree));
        ...
        Path jar=Path.of(System.getProperty("wsr.comparative.scoring.browser.jar")).toRealPath();
        assertThat(jar.startsWith(root.resolve(".cache").toRealPath())).isTrue();
        var logs=Files.createTempDirectory(root.resolve(".cache"),"adr085-evidence-");
        child("node",List.of("node_modules/next/dist/bin/next","build"),web,logs.resolve("build.log"),Map.of(),120);
        try(var db=new PostgreSQLContainer<>("postgres:17-alpine").withDatabaseName("wsr_comparative_scoring_demo").withUsername("receipt_demo_owner")
                .withPassword("DisposableDatabaseOnly").withReuse(false).withLabel("com.wallstreetreceipts.comparative-scoring-browser","ADR-085")
            ...
            owner.execute("CREATE ROLE receipt_comparative_append LOGIN PASSWORD 'DisposableAppendOnly'");
            owner.execute("GRANT USAGE ON SCHEMA public TO receipt_comparative_append");
            owner.execute("GRANT SELECT ON ALL TABLES IN SCHEMA public TO receipt_comparative_append");
            owner.execute("GRANT INSERT ON demo_comparative_scoring_receipts TO receipt_comparative_append");
            owner.execute("GRANT UPDATE(call_id) ON analyst_calls TO receipt_comparative_append");
            var appendRole=new JdbcTemplate(new DriverManagerDataSource(db.getJdbcUrl(),"receipt_comparative_append","DisposableAppendOnly"));
            assertThat(appendRole.queryForObject("SELECT has_table_privilege(current_user,'demo_comparative_scoring_receipts','UPDATE,DELETE')",Boolean.class)).isFalse();
            try(var context=(ServletWebServerApplicationContext)new SpringApplicationBuilder(WallStreetReceiptsApiApplication.class).run(
                ...
                assertThatThrownBy(()->reader.update("DELETE FROM demo_comparative_scoring_receipts WHERE FALSE")).isInstanceOf(org.springframework.dao.DataAccessException.class);
                ...
                var input=ComparativeScoringFixture.input(ScoringReceiptFixture.input(call));Path file=logs.resolve("explicit-demo-input.wsr");Files.write(file,ComparativeScoringInputCodec.encode(input));
                ...
                for(var candidate:List.of(ComparativeScoringFixture.input(edit(input.endpoint(),"evaluationAsOf",BASIS.plusSeconds(1))),
                        edit(input,"benchmark",edit(input.benchmark(),"endpointLevelCandidates",List.of())),
                        ComparativeScoringFixture.input(ScoringReceiptFixture.corrected(input.endpoint(),revision)))) {
                    var candidateFile=logs.resolve(UUID.randomUUID()+".wsr");Files.write(candidateFile,ComparativeScoringInputCodec.encode(candidate));append(jar,candidateFile,logs.resolve(UUID.randomUUID()+".log"),db);
                }
                assertThat(owner.queryForObject("SELECT COUNT(*) FROM demo_comparative_scoring_receipts",Integer.class)).isEqualTo(4);
                var seeded=inventory(owner);var oldTables=new TreeMap<>(seeded);oldTables.put("demo_comparative_scoring_receipts",empty.get("demo_comparative_scoring_receipts"));assertThat(oldTables).isEqualTo(empty);
                browser(web,logs,context.getWebServer().getPort(),"ready",saved[0],saved[1]);assertThat(inventory(owner)).isEqualTo(seeded);
                owner.execute("REVOKE SELECT ON demo_comparative_scoring_receipts FROM receipt_ui_reader");
                browser(web,logs,context.getWebServer().getPort(),"unavailable",saved[0],saved[1]);assertThat(inventory(owner)).isEqualTo(seeded);
                owner.execute("GRANT SELECT ON demo_comparative_scoring_receipts TO receipt_ui_reader");
        ...
        System.out.println("ADR-085 DEMO PASS: packaged explicit append, idempotent retry, unchanged parents and SELECT-only production UI; evidence "+logs);
    }
    private static String[] append(Path jar,Path input,Path log,PostgreSQLContainer<?> db) throws Exception {
        ...
        child(java,List.of("-Dloader.main=com.wallstreetreceipts.api.application.scoring.DemoComparativeScoringReceiptCommand","-cp",jar.toString(),
                "org.springframework.boot.loader.launch.PropertiesLauncher","--confirm=APPEND_DEMO_COMPARATIVE_SCORING_RECEIPT","--input="+input,"--snapshot=receipt-snapshot"),input.getParent(),log,
                Map.of("WSR_DEMO_COMPARATIVE_SCORING_JDBC_URL","jdbc:postgresql://127.0.0.1:"+db.getMappedPort(5432)+"/wsr_comparative_scoring_demo","WSR_DEMO_COMPARATIVE_SCORING_USER","receipt_comparative_append","WSR_DEMO_COMPARATIVE_SCORING_PASSWORD","DisposableAppendOnly"),30);
        String output=Files.readString(log).strip();assertThat(output).matches("DEMO_COMPARATIVE_SCORING_RECEIPT: [0-9a-f-]{36} [0-9a-f]{64}");
        return output.substring("DEMO_COMPARATIVE_SCORING_RECEIPT: ".length()).split(" ");
    }
    private static void browser(Path web,Path logs,int api,String phase,String id,String hash) throws Exception {
        ...
        var env=new HashMap<String,String>();env.put("WSR_COMPARATIVE_SCORING_UI_PORT",""+port);env.put("WSR_COMPARATIVE_SCORING_API_PORT",""+api);env.put("WSR_COMPARATIVE_SCORING_PHASE",phase);
        if(id!=null){env.put("WSR_COMPARATIVE_SCORING_RECEIPT_ID",id);env.put("WSR_COMPARATIVE_SCORING_INPUT_HASH",hash);}
        child("node",List.of("node_modules/@playwright/test/cli.js","test","--config","comparative-scoring/full-stack.config.ts"),web,logs.resolve(phase+".log"),env,180);
    ...
    private static void publicBrowser(Path web,Path logs) throws Exception {
        int port;try(var socket=new ServerSocket(0,1,InetAddress.getByName("127.0.0.1"))){port=socket.getLocalPort();}
        child("node",List.of("node_modules/@playwright/test/cli.js","test","--config","comparative-scoring/public.config.ts"),web,logs.resolve("public.log"),
                Map.of("WSR_COMPARATIVE_SCORING_UI_PORT",""+port),900);
        try(var socket=new ServerSocket(port,1,InetAddress.getByName("127.0.0.1"))){assertThat(socket.isBound()).isTrue();}
    }
