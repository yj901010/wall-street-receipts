...
@ActiveProfiles("test")
class AnalystCallOutcomeFailureApiTest {
    ...
    @Test
    void repositoryFailureUsesTheClosedInternalErrorProblemWithoutLeakingDetails() throws Exception {
        when(outcomeRepository.findByCallId("demo-call-001"))
                .thenThrow(new IllegalStateException("forced repository failure"));

        MvcResult result = mockMvc.perform(get("/v1/calls/demo-call-001/outcomes")
                        .header("X-Request-Id", "req-outcome-failure"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
                .andExpect(header().string("X-Request-Id", "req-outcome-failure"))
                .andExpect(jsonPath("$.code").value("INTERNAL_ERROR"))
                .andExpect(jsonPath("$.detail").value("An unexpected server error occurred."))
                .andExpect(jsonPath("$.detail").value(org.hamcrest.Matchers.not(
                        org.hamcrest.Matchers.containsString("forced repository failure"))))
                .andExpect(jsonPath("$.requestId").value("req-outcome-failure"))
                .andReturn();
