...
marker = (
    "ADR-032 calculates a signed sector price-index return from one "
    "complete ADR-030 sector reference-level-pair receipt using the "
    "exact basis-level denominator."
)
adr_path = Path(
    "decisions/ADR-032-signed-sector-reference-return.md"
)
marker_paths = (
    adr_path,
    Path("README.md"),
...
for path in marker_paths:
    require(path.is_file(), f"Missing ADR-032 contract document: {path}")
    require(
        path.read_text(encoding="utf-8").count(marker) == 1,
        f"ADR-032 marker must occur exactly once: {path}",
    )
...
require(
    ...
    "ADR-032 title, accepted status, or date changed",
)
...
test_files = {"SectorReturnCalculatorGoldenTest.java"}
require(
    {path.name for path in production_dir.glob("*.java")}
    == production_files,
    "ADR-032 production surface must remain exactly four files",
)
require(
    {path.name for path in test_dir.glob("*.java")} == test_files,
    "ADR-032 test surface must remain exactly one source-local golden",
)
sources = {
...
expected_source_hashes = {
    "SectorReturnCalculator.java":
        "d7bab170807c8b11aa8a842523db475eeb12e2e753f1e229832111a99a7c6d06",
    "SectorReturnInput.java":
        "c8361ff161b6be2d621feb400cde8a2b025d1038abf76f3fccf70ec09091f361",
    "SectorReturnPolicyVersion.java":
        "56d7789f67162227a8783081bd7f5f4f686a7b948b39d7ad98342fc7abf08f64",
    "SectorReturnResult.java":
        "b63f136134a91b7a22d673ff1e722c89fd17407822e02437f4066ca57221a46f",
}
require(
    ...
    "ADR-032 exact normalized production source changed",
)

policy_source = sources["SectorReturnPolicyVersion.java"]
...
require(
    ...
    "ADR-032 must preserve its formula and one exact canonical policy block",
)
policy = json.loads(definition, object_pairs_hook=reject_duplicate_keys)
require(
    len(definition.encode("ascii")) == 2817
    ...
    and enum_values(policy_source, "SectorReturnPolicyVersion") == [
        "SIGNED_SECTOR_BASIS_LEVEL_DENOMINATOR_SCALE_12_HALF_EVEN_V1"
    ]
    and definition.count(pair_hash) == 1,
    "ADR-032 canonical 2817 bytes, hash, enum, or required pair hash changed",
)
expected_variants = {
    "Available": ["context", "sectorReturn"],
...
require(
    policy["requiredReferenceLevelPairPolicyVersion"]
        == "POINT_IN_TIME_EXACT_SECTOR_PRICE_INDEX_LEVEL_PAIR_V1"
    and policy["requiredReferenceLevelPairPolicyDefinitionHash"]
        == pair_hash
    ...
    "ADR-032 exact context, six variants, branch map, or nested reason rule changed",
)
require(
    ...
    and policy["benchmarkReturnResultReuse"] == "FORBIDDEN"
    and policy["reflectionOrClassTokenUse"] == "ABSENT",
    "ADR-032 formula, decimal, isolation, or source boundary changed",
)

input_source = sources["SectorReturnInput.java"]
result_source = sources["SectorReturnResult.java"]
calculator_source = sources["SectorReturnCalculator.java"]
require(
    record_fields(input_source, "SectorReturnInput") == [
    ...
    and enum_values(result_source, "OutputUnavailableReason")
        == ["OUTPUT_NOT_REPRESENTABLE"],
    "ADR-032 public record fields or sole output reason changed",
)
permitted = re.search(
    r"sealed interface SectorReturnResult\s+permits(?P<body>.*?)\{",
    result_source,
    flags=re.DOTALL,
)
require(
    permitted is not None
    and re.findall(r"SectorReturnResult\.([A-Z][A-Za-z]+)",
                   permitted.group("body"))
        == list(expected_variants),
    "ADR-032 sealed result variant order changed",
)
require(
    input_source.count(pair_hash) == 1
    and sum(source.count(pair_hash) for source in sources.values()) == 2,
    "ADR-032 required ADR-030 policy identity must remain exact",
)
...
formula_tokens = (
    ...
    "sectorReturn=numerator.divide(basis,OUTPUT_SCALE,"
...
require(
    ...
    and "sectorReturn.compareTo(BigDecimal.ONE.negate())<0"
        in calculator_logic
    and "sectorReturn.scale()!=OUTPUT_SCALE" in calculator_logic
    and "sectorReturn.precision()>OUTPUT_PRECISION"
        in calculator_logic,
    "ADR-032 must subtract exactly before its sole scale-12 HALF_EVEN division",
)
branch_positions = [
    calculator_logic.find(
        f"caseSectorReferenceLevelPairResolution.{name}"
    )
    for name in (
...
require(
    ...
    "ADR-032 calculator branch propagation or complete receipt context changed",
)
...
require(
    imported_outcome_packages
        <= {"sectorreferencepair", "sectorreturn"}
    and "AssetReturn" not in all_logic
    and "BenchmarkReturn" not in all_logic
    and "SectorAssignment" not in all_logic
    and "EndpointPrice" not in all_logic
    and "OutcomeBasis" not in all_logic
    and ".reason()" not in all_logic
    and "SectorReferenceLevelPairResolution.UnavailableReason"
        not in all_logic
    and "SectorReferenceLevelPairResolution.EndpointAnchorUnavailableReason"
        not in all_logic
    and not re.search(
    ...
    "ADR-032 must remain a no-reflection independently typed pure pair consumer",
)

golden_path = test_dir / "SectorReturnCalculatorGoldenTest.java"
...
required_methods = (
    ...
    "propagatesEveryNotApplicableBranchOnlyInsideTheCompletePairReceipt",
...
require(
    ...
    and len(re.findall(r"(?m)^\s+@EnumSource\(", golden)) == 4
    and len(re.findall(r"(?m)^\s+@MethodSource\(", golden)) == 2
    and "@EnumSource(SectorReferenceLevelPairResolution.UnavailableReason.class)"
        in golden
    and "@EnumSource(SectorReferenceLevelPairResolution.EndpointAnchorUnavailableReason.class)"
        in golden
    and "@EnumSource(SectorAssignmentResolution.UnavailableReason.class)"
        in golden
    and "@EnumSource(SectorAssignmentResolution.NotApplicableReason.class)"
        in golden
    ...
    and "newSectorReturnResult.NotApplicable(resolvedContext)"
        in golden_logic
    and "newSectorReturnResult.AssignmentUnavailable(resolvedContext)"
        in golden_logic
    and "newSectorReturnResult.EndpointAnchorUnavailable(resolvedContext)"
        in golden_logic
    and "ObjectMapper" not in golden
    and "ClassPathResource" not in golden,
    "ADR-032 source-local 112-vector golden contract changed",
)
...
allowed_readiness_edges = {
    ...
    (sector_readiness_dir
     / "SectorReturnReadinessRequest.java").resolve(): {
        "SectorReturnPolicyVersion", "SectorReturnResult",
    },
    (sector_readiness_dir
     / "SectorReturnReadinessResolution.java").resolve(): {
        "SectorReturnResult",
    },
    (sector_readiness_dir
...
for path in Path("apps/api/src/main/java").rglob("*.java"):
    ...
    if path.resolve() in sector_readiness_paths:
        ...
        require(
            ...
            and "domain.outcome.benchmarkreturn" not in source,
            f"ADR-033 sector readiness return edge changed: {path}",
        )
        continue
    require(
        "domain.outcome.sectorreturn" not in source
        and not any(re.search(rf"\b{re.escape(name)}\b", logic)
                    for name in type_markers),
        f"ADR-032 sector-return leaf must not be reverse-wired: {path}",
    )
...
require(
    ...
    "ADR-032 current protected production baseline changed",
)
...
adr030_protected_paths = [
    ...
    if path.resolve() not in adr031_production_paths
]
require(
    len(adr031_production_paths) == 4
    and {path.resolve() for path in adr031_protected_paths}
    ...
    "ADR-030 protected baseline changed outside ADR-031/032 production",
)
...
require(
    ...
    and {path.resolve() for path in adr030_protected_paths}
    - {path.resolve() for path in adr029_protected_paths}
        == adr030_production_paths
    ...
    "ADR-029 protected baseline changed outside ADR-030/031/032 production",
)
...
require(
    ...
    "ADR-032 current API-test/web baseline changed",
)
...
require(
    ...
    "ADR-030 test/web baseline changed outside ADR-031/032 goldens",
)
adr030_golden_paths = {
    Path(
...
require(
    {path.resolve() for path in adr030_test_web_paths}
    - {path.resolve() for path in adr029_test_web_paths}
        == adr030_golden_paths
    and len(adr029_test_web_paths) == 199
    and digest(adr029_test_web_paths)
        == "5c09f8859707bd8fbe59aaa2735d066fe8eedb730068da0f3e52d33a9c5907df",
    "ADR-029 test/web baseline changed outside ADR-030/031/032 goldens",
)
...
runtime_markers = (
    "SIGNED_SECTOR_BASIS_LEVEL_DENOMINATOR_SCALE_12_HALF_EVEN_V1",
    "domain.outcome.sectorreturn",
    "SectorReturnCalculator",
    "SectorReturnResult",
)
for path in product_paths:
    ...
    require(
        ...
        f"ADR-032 sector return must not be published: {path}",
    )
outcomes = json.loads(
...
require(
    ...
    "ADR-032 must not publish comparative returns or alpha",
)

print(
    "Validated ADR-032 exact 4+1 surface, 2817 canonical bytes, "
    "subtraction-before-division scale-12 HALF_EVEN formula, exact six-way "
    "complete-receipt propagation, 56/3 nested reason replay, 112-vector "
    "golden, reverse isolation, current baselines, ADR-031/030/029 replay, "
