...
CUSTODY_MANIFEST_DIGEST = "b3e5cd22650cdde13b7ea24753cd9068bdd6a08669166e052a1ab399fbbad824"  # ADR056_CUSTODY_MANIFEST_DIGEST
...
require(
    ...
    "ADR-056 original HEAD custody is invalid",
)
...
require(
    ...
    "ADR-056 excluded Next declaration custody is invalid",
)
...
require(
    ...
    "ADR-056 current-byte custody manifest is missing",
)
...
require(
    isinstance(raw_manifest, dict) and len(raw_manifest) == 6,
    "ADR-056 current-byte custody inventory changed",
)
current_files = {
...
require(
    ...
    "ADR-056 modified/added custody partition changed",
)
require(
...
for path, metadata in current_files.items():
    require(
        ...
        and re.fullmatch(r"[0-9a-f]{64}", metadata["rawSha256"]),
        f"ADR-056 custody metadata is invalid: {path}",
    )
    if metadata["kind"] == "self":
        require(
            path == workflow_path
            and metadata["normalizedSha256"] is None,
            f"ADR-056 workflow self-custody metadata changed: {path}",
        )
    else:
        require(
            path != workflow_path
            ...
            and re.fullmatch(
                r"[0-9a-f]{64}", metadata["normalizedSha256"]
            ),
            f"ADR-056 text custody hash is invalid: {path}",
        )
    custody_path = projection_root / path
    require(
        custody_path.is_file(),
        f"ADR-056 custody is missing: {path}",
    )
    if metadata["kind"] == "self":
        require(
        ...
        require(
            ...
            f"ADR-056 current-byte custody changed: {path}",
        )
...
require(
    ...
    "ADR-056 projected HEAD state could not be resolved",
)
if observed_head == BASE_REVISION:
    if observed_symbolic_head.returncode != 1:
        projection_errors.append(
            "ADR-056 historical HEAD was not detached after nested guards"
        )
    ...
    if tracked_status != (
        ...
        projection_errors.append(
            "ADR-056 historical tracked checkout changed during nested guards"
        )
    for path in added_paths:
        if path.exists():
            projection_errors.append(
                f"ADR-056 added path reappeared during historical guards: {path}"
            )
elif observed_head == original_head:
    print(
        "ADR-056 projection stopped after custody but before the base "
        "checkout; restoring the original repository state"
    )
else:
    projection_errors.append(
        "ADR-056 nested guards left HEAD at an unexpected commit: "
        + observed_head
    )
...
if original_symbolic_ref is not None:
    ...
    require(
        ...
        "ADR-056 original branch moved during historical guards",
    )
    subprocess.run(
...
require(
    ...
    "ADR-056 original commit/symbolic HEAD state was not restored",
)
...
for path, metadata in current_files.items():
    require(
        path.is_file(),
        f"ADR-056 current path was not restored: {path}",
    )
    custody_path = projection_root / path
    if metadata["kind"] == "self":
        require(
            ...
            "ADR-056 workflow bytes were not restored exactly",
        )
    else:
        require(
            ...
            == metadata["normalizedSha256"],
            f"ADR-056 current bytes were not restored exactly: {path}",
        )
if next_env_dirty:
    shutil.copyfile(next_env_custody_path, next_env_path)
...
require(
    ...
    "ADR-056 restored checkout is not tracked-clean outside the "
...
(projection_root / ".restored").write_text(
    "ADR-056 exact current bytes restored after historical guards\n",
    encoding="utf-8",
)
require(not projection_errors, "\n".join(projection_errors))
print(
    "Restored and verified all 6 exact ADR-056 "
    "source/script/document file surfaces"
)
PYTHON
