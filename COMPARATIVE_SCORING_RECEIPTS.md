# DEMO comparative scoring receipt creation and audit
...
Open `/calls/{callId}/comparative-scoring-receipts`, or follow **DEMO comparative scoring receipts** from a
...
```text
COMPARATIVE_SCORING_RECEIPTS_PROVIDER=api
...
replay and ledger verification are performed by Java's ADR-084 service.
...
agreement and target error; it remains PARTIAL_COMPARATIVE with dataComplete=false.
...
1. Prepare a **separate disposable DEMO PostgreSQL database** named exactly
   `wsr_comparative_scoring_demo`, bound to `127.0.0.1` on an explicit port (1024–65535).
   Apply the existing migrations through V13 using your normal approved database
...
3. Supply a bounded canonical `.wsr` input file produced by
   `ComparativeScoringInputCodec.encode(ComparativeScoringInput)`. This is the exact
   versioned ADR-083 binary format, not Java serialization, JSON or arbitrary
...
```text
WSR_DEMO_COMPARATIVE_SCORING_JDBC_URL=jdbc:postgresql://127.0.0.1:55432/wsr_comparative_scoring_demo
WSR_DEMO_COMPARATIVE_SCORING_USER=<dedicated local DEMO database role>
WSR_DEMO_COMPARATIVE_SCORING_PASSWORD=<local password supplied privately>
```
...
referenced ledger tables, SELECT/INSERT on demo_comparative_scoring_receipts and a suitable
...
```text
java -Dloader.main=com.wallstreetreceipts.api.application.scoring.DemoComparativeScoringReceiptCommand -cp /absolute/path/api.jar org.springframework.boot.loader.launch.PropertiesLauncher --confirm=APPEND_DEMO_COMPARATIVE_SCORING_RECEIPT --input=/absolute/path/explicit-demo-input.wsr --snapshot=receipt-snapshot
```
...
`ComparativeScoringReceiptBrowserIT` is explicitly opt-in after packaging the current API.
Use `-Dtest=ComparativeScoringReceiptBrowserIT`, `-Dwsr.comparative.scoring.browser.confirm=DISPOSABLE_DEMO_ONLY`,
`-Dwsr.comparative.scoring.browser.web=<secret-free current Web mirror under repository .cache>`,
and `-Dwsr.comparative.scoring.browser.jar=<fresh packaged API under repository .cache>`.
The mirror must include current src/public/operator/scoring/comparative-scoring/e2e, Web build config,
...
ignored `.cache/adr085-evidence-*` logs/inputs and screenshots remain for inspection.
